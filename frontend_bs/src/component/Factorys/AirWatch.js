...
function DateAndTimeForm({ setData, isLoading, setIsLoading, data }) {
  ...
  useEffect(() => {
    axios
      .get(
        `http://junlab.postech.ac.kr:880/api2/factory/${factoryId}/airwatches`
      )
      .then((response) => {
  ...
  const handleSubmit = (e) => {
    ...
    if (isLoading) {
      ...
      axios
        .get(
          `http://junlab.postech.ac.kr:880/api2/airwatchdata?start=${startDate}&end=${endDate}&factoryId=${factoryId}&${selected.type}=${selected.value}`
        )
        .then((response) => {
          setIsLoading(false);
